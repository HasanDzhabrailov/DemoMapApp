...
import ru.tech.demomapapp.feature.map.router.MapRouterStore
import ru.tech.demomapapp.feature.map.router.MapRouterStoreFactory
import ru.tech.demomapapp.feature.map.router.MapRouterStoreHolder
...
fun createMapHostComponent(
    componentContext: ComponentContext,
    initialModel: MapScreenComponent.Model = MapScreenComponent.Model(),
): MapScreenUiContract = DefaultMapHostComponent(
    componentContext = componentContext,
...
internal class DefaultMapHostComponent(
    ...
    override fun onMapToolsClick() {
        routeOverlay(MapRouterStore.OverlayTarget.TOOLS_OVERLAY, toolsComponent::onMapToolsClick)
    }

    override fun onAvailableMapsClick() {
        routeOverlay(MapRouterStore.OverlayTarget.TOOLS_OVERLAY, toolsComponent::onAvailableMapsClick)
    }

    override fun onMapsOnScreenClick() {
        routeOverlay(MapRouterStore.OverlayTarget.TOOLS_OVERLAY, toolsComponent::onMapsOnScreenClick)
    }
    ...
    override fun onCreatePointClick() {
        routeOverlay(MapRouterStore.OverlayTarget.DRAWING_OVERLAY, drawingComponent::onCreatePointClick)
    }

    override fun onCreateLineClick() {
        routeOverlay(MapRouterStore.OverlayTarget.DRAWING_OVERLAY, drawingComponent::onCreateLineClick)
    }

    override fun onCreatePolygonClick() {
        routeOverlay(MapRouterStore.OverlayTarget.DRAWING_OVERLAY, drawingComponent::onCreatePolygonClick)
    }

    override fun onDrawingAddPositionClick() {
        routeOverlay(
            MapRouterStore.OverlayTarget.VIEWPORT_EXCLUSIVE_ACTION,
    ...
    private fun routeOverlay(target: MapRouterStore.OverlayTarget) {
        routerHolder.accept(MapRouterStore.Intent.OverlayInteractionRequested(target))
    }

    private fun routeOverlay(target: MapRouterStore.OverlayTarget, action: () -> Unit) {
        routeOverlay(target)
        action()
    }
